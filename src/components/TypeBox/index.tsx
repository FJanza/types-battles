...
export default function TypeBox({wordsQuantity, timeGame}: TypeBoxProps) {
  const INITIAL_TIME = timeGame; //segundos
  ...
  function initGame() {
    ...
    words = WORDS.toSorted(() => Math.random() - 0.5).slice(0, wordsQuantity);
  ...
  function onKeyUp() {
    ...
    if ($nextActiveLetter) {
      ...
      if (!$nextWord) gameOver(Number($time.current?.textContent));
    }
  }

  function gameOver(extraTime: number) {
    const correctWords =
    ...
    const score = correctLetter + extraTime;

    setGameOverData({wpm, accuracy: accuracy.toFixed(2), score});
